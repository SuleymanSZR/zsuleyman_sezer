*&---------------------------------------------------------------------*
*& Include          ZSULEYMAN_SEZER_0007_CLS
*&---------------------------------------------------------------------*
CLASS lcl_report DEFINITION.
  ...
  PROTECTED SECTION.
    ...
    DATA : lv_fm_name TYPE rs38l_fnam.     "smartform için
...
**Netwr alanı için toplamları.
    SELECT SUM( netwr )
      FROM vbap
      INTO @gv_sum
      WHERE vbap~vbeln EQ @lv_vbeln.

    fp_outputparams-device      = 'PRINTER'.
*    fp_outputparams-nodialog    = 'X'.  "görünüm açar X lenirse popup ı kapatır.
    fp_outputparams-preview     = 'X'.   "yazdırma görünümünü gösterir
*    fp_outputparams-nopreview     = 'X'.   "yazdırma görünümünü göstermez
    fp_outputparams-dest        = 'LP01'.   "sabit yazıcı
...
* Implement suitable error handling here
    ...
    CALL FUNCTION fm_name
      EXPORTING
        ...
        is_header         = gs_header   "sipariş başlık bilgileri tutan str
        it_items          = gt_items    "str nin kalem bilgilerini tutan table
        iv_sum            = gv_sum
*   IMPORTING
*       /1BCDWB/FORMOUTPUT       =
      EXCEPTIONS
...
* Implement suitable error handling here
  ...
  METHOD print_smartform.
    ...
    CALL FUNCTION 'SSF_FUNCTION_MODULE_NAME'
      EXPORTING
        formname           = 'ZVKT_SS_SF_0002' "Kullanılacak smartforms adı
      IMPORTING
        fm_name            = lv_fm_name
      EXCEPTIONS
        no_form            = 1
    ...
    SELECT SUM( netwr )    "bu şekilde toplamı gönderince
      FROM vbap            "toplam değeri her sayfada basıyor
      INTO @gv_sum         "formun içinde yapınca sayfa sayfa toplam yaparx
      WHERE vbap~vbeln EQ @lv_vbeln.
...
*       user_settings    = ' '
        is_header        = gs_header   "forma basılacak olan bilgilerin çıkilmesine yarayacak alv deki bilgiler
        it_items         = gt_items
        iv_sum           = gv_sum
        iv_belnr         = lv_vbeln
      EXCEPTIONS
